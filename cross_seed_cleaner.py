...
class _BoolFlag(argparse.BooleanOptionalAction):
    """BooleanOptionalAction with a separate description for the --no-* form."""
    def __init__(self, *args, no_help=None, **kwargs):
...
class _SplitBoolHelpFormatter(argparse.HelpFormatter):
    ...
    def _format_action(self, action):
        ...
        no_help = getattr(action, "no_help", None)
        parts = []
        for flag in action.option_strings:
            text = no_help if flag.startswith("--no-") else action.help
            text = self._expand_help(action) if text is action.help and text else text
            if not text:
                parts.append('%*s%s\n' % (self._current_indent, '', flag))
                continue
            lines = self._split_lines(text, help_width)
            if len(flag) <= action_width:
                parts.append('%*s%-*s  %s\n' % (self._current_indent, '', action_width, flag, lines[0]))
                lines = lines[1:]
            else:
                parts.append('%*s%s\n' % (self._current_indent, '', flag))
            for line in lines:
...
def _reason_text(code):
    ...
    if code == "NO_ADDED_TIME": return "Torrent has no added_on timestamp; cannot apply min-age check safely"
                    ...
                    <div class="grid-row grid-filterrow filter-row">
                        <div class="fcell" data-col="0">
                            ...
                            <div class="filter-multi-panel" data-filter-panel="status">
                                ...
                                <label><input type="checkbox" value="RECENTLY_ADDED" data-filter-reason>⏰ Recently added</label>
        ...
        try:
            ...
            with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for idx, (h, d) in enumerate(sorted_items, 1):
                    ...
                    for i, t in enumerate(group_torrents):
...
def _finalize_deletion(client, emap):
    ...
    if SCAN_STATS.get('scan_incomplete') and not DRY_RUN:
        ...
        # (manual-live or auto-live); the report is already written, so the user
        # can review and re-run after resolving the error. ANY dry-run — plain or
        # --manual — deletes nothing (delete_torrents returns "dry_run"), so it
        # falls through rather than claiming deletion was "disabled" for a run
        # that was never going to delete anyway (and a manual dry-run still gets
        # its interactive preview loop).
