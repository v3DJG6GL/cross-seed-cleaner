"""
Cross-Seed Cleaner — local overrides template.

Copy this file to config.local.py (same directory) and un-comment ONLY the
settings you want to change from the config.py defaults. config.local.py is
gitignored, so a git pull never conflicts with your edits, while new settings
added upstream still arrive in config.py with their defaults and comments.

Every setting below mirrors config.py (same names, same defaults, commented
out). Precedence: config.py < config.local.py < environment variables < CLI
flags. Set LOCAL_CONFIG=/path/to/file to load the overrides from elsewhere,
or LOCAL_CONFIG="" to skip them entirely.
"""
